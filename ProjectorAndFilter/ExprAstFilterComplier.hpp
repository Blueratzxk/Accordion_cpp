...
#ifndef OLVP_EXPRASTFILTERCOMPLIER_HPP
#define OLVP_EXPRASTFILTERCOMPLIER_HPP



#include "../Nodes/Node/AstNodeVisit.hpp"
#include "../Nodes/Node/AstNodeVisitor.hpp"
...
class ExprAstFilterComplier: public AstNodeVisitor
{
...
public:
    ...
    void* VisitFunctionCall(FunctionCall* node,void* context) {
        ...
        if(node->getFuncName() == "and")
        {
            functionNode = TreeExprBuilder::MakeAnd(functionArguments);
        }
        else if(node->getFuncName() == "or")
        {
            functionNode = TreeExprBuilder::MakeOr(functionArguments);
        }
        else {

            functionNode = TreeExprBuilder::MakeFunction(node->getFuncName(), functionArguments,Typer::getType(node->getOutputType()));
        }
...
#endif //OLVP_EXPRASTFILTERCOMPLIER_HPP
