...
Accordion can tune the parallelism of a query at any time during its execution, and the tuning process will not affect the query execution. Its what-if service facilitates parallelism tuning and can predict DOP-tuned remaining query execution times for users. Accordion can be used to solve the problem of cost-performance tradeoffs when analyzing date on the cloud. It also can be used to build a full-serverless cloud database or data warehouse.
